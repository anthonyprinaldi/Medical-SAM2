...
class NEUROSAM(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(num_frame):
            data_seg_3d[..., i] = np.load(mask_path / f"{i}.npy")
            ...
            for obj in obj_list:
                ...
                if self.prompt == "click":
                    diff_obj_point_label_dict[obj], diff_obj_pt_dict[obj] = (
                        random_click(
                            np.array(obj_mask.squeeze(0)), point_label, seed=None
                        )
                    )
                if self.prompt == "bbox":
                    diff_obj_bbox_dict[obj] = generate_bbox(
                        np.array(obj_mask.squeeze(0)),
